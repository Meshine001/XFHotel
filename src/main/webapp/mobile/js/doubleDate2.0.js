	...
	function kui_date(){ //var d = new Date().getTime();
		...
		if(txt_left + 370 < scrollWidth){
			...
			if($(document).clientHeight - txt_top < 217 && $(document).clientHeight > 217){
				$('#kui_d_pane').attr('style','left:'+ 0 +'px; top:'+ (kDate.offset().top - 197) +'px;');
			}
			else{
				$('#kui_d_pane').attr('style','left:'+ 0 +'px; top:'+ txt_top +'px;');
			}
		}
		else{
		...
		//$('.doubledate').eq(0).attr('placeholder' , kui_year+'-'+kui_month+'-'+kui_date);
		// 上月下月
