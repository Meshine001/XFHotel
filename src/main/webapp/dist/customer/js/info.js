	   ...
	   try{
	       ...
	       calendarV2.prototype.fillCalender = function() {
	           // this.e.html(this.prependHtml + this.calenderHTML +
				// this.clearDateHTML() + this.preMonthHTML() +
				// this.nextMonthHTML());
	           var nationHtml = '';
	           if(this.isAbroad){
	           ...
	           // this.e.find('.' +
				// this.classMonth).first().addClass('paddingR10');
...
// })
	       ...
	       calendarV2.prototype.refreshCheckState = function() {
	           ...
	           /*
				 * this.e.find('.' +
				 * this.classDayBase).not('.old').not('span').each(function(){
				 * var liYmd = $(this).attr('ymd'); liYmd = new
				 * Date(liYmd.replace(/-/g, "/")); if (_this.doColor) {
				 * if(!_this.checkIn) { if
				 * ($(this).hasClass(_this.classDaySelect)) doColorState =
				 * !doColorState; if (doColorState){
				 * $(this).addClass(_this.classDaySelect);
				 * $(this).addClass('checkedday'); if (_this.getDayInfo)
				 * _this.getDayInfo($(this)); } } else { if(
				 * (_this.getYmd(liYmd) >= _this.getYmd(_this.checkIn)) && (
				 * _this.getYmd(liYmd) <= _this.getYmd(_this.checkOut)) ) {
				 * $(this).addClass(_this.classDaySelect);
				 * $(this).addClass('checkedday'); if (_this.getDayInfo)
				 * _this.getDayInfo($(this)); } } } if(_this.getYmd(liYmd) <
				 * _this.getYmd(_this.checkIn) ) {
				 * $(this).addClass(_this.classDayUnSelectAble);
				 * $(this).removeClass(_this.classDayShow); } if
				 * (_this.fillDayInfo) _this.fillDayInfo($(this)); })
				 */
	           this.checkDayChange();
	           return(this);
	       };
...
$(document).ready(function(){	
    ...
    $('#startenddate').click(function(){
           var  detailCalendar = new execCalendar('#startenddate',{autoSearch:true,showPrice:true,calendar:'#calendar-box'});
           $('#calendar-box').show();
    });
